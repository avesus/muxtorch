...
class one_layer_net (torch.nn.Module):    
    ...
    def __init__ (self):
        ...
        torch.nn.init.normal_(self.l1.bias, mean=0, std=1.0/2.0)
        #torch.nn.init.normal_(self.l2.bias, mean=0, std=1.0/8.0)
        #torch.nn.init.normal_(self.l3.bias, mean=0, std=1.0/8.0)
        torch.nn.init.normal_(self.l2.bias, mean=0, std=1.0)
    ...
    def forward (self, x) :
        ...
        '''
        torch_tensors = torch.nn.functional.sigmoid(
          #self.l4(
...
for epoch in range(epochs):
    ...
    #for i in np.random.permutation(range(len(training_data))) :
      ...
      network = -1.0 * torch.mean(y * torch.log(yhat) + (1.0 - y) * torch.log(1.0 - yhat))
      #network = network - y * torch.log(yhat) + (1.0 - y) * torch.log(1.0 - yhat)
